...
texture targetTex <string uiname="Target";>;
sampler target = sampler_state    //sampler for doing the texture-lookup
{
    Texture   = (targetTex);          //apply a texture to the sampler
...
float4 PS_flowAdvect(vs2ps In): COLOR
{
	float2 pos    = In.TexCd;
  	...
  	float4 newValue = tex2D(target, pos);  
